    ...
    def forward(self, input):
        ...
        for s in range(sout):
            ...
            if s<= input.size()[2]-1:
                ...
                x = input.reshape(input.size()[0], -1, input.size()[3], input.size()[4])
            # Cut out the weights
...
class BesselConv2d(nn.Module):
    ...
    """
    def __init__(self, n_channels=3,base=2., zero_scale=0.5, n_scales=8, scales=None,device=None):
        ...
        if self.scales != []:
            ...
            if self.device is not None:
                for k in range(len(self.weights)):
                    self.weights[k][0].to(self.device)
                    self.weights[k][1].to(self.device)
                #self.weights.to(self.device)
            output = [F.conv2d(input, self.weights[d][0].to(self.device), bias=None,
                   padding=(0,pad[d]), stride=1, dilation=1) for d in range(len(pad))]
            output = [F.conv2d(output[d], self.weights[d][1].to(self.device), bias=None,
